  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (modeNum == 1 || modeNum == 3) {
      ...
      if (
        ball.isDownPowerhit &&
        ball.isServeState &&
        !ball.expectedNetCollision &&
        ((ball.expectedLandingPointX >= GROUND_HALF_WIDTH &&
          !ball.isPlayer2Serve) ||
          (ball.expectedLandingPointX < GROUND_HALF_WIDTH &&
            ball.isPlayer2Serve))
      ) {
...
function processPlayerMovementAndSetPlayerPosition(
  ...
  if (
    modeNum === 3 &&
    ((!player.isPlayer2 && ball.dribbleCounts[0] > 2) ||
      (player.isPlayer2 && ball.dribbleCounts[1] > 2))
  ) {
    ball.canPowerhitBasedOnCollision = false;
  } else if (modeNum == 3) {
    ball.canPowerhitBasedOnCollision = true;
